...
check_different_url() {
  last_url=`tail -1 /tmp/urls$2.log`
  if [ "$last_url" == "$1" ]
  then
    false
  else
    echo "$1" >> /tmp/urls$2.log 2>&1
    true
  fi
}
...
get_server_auction () {
  ...
  url=`curl --header "Authorization: Bearer ${token}" https://eu.api.blizzard.com/wow/auction/data/$1?locale=en_US | jq -j .files[0].url`
  if check_different_url "$url" "$1"
  then
